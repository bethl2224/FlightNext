...
const HotelSearchBar: React.FC<HotelSearchBarProps> = ({addHotelToCart }) => {
  ...
  const handleFlightSuggestions = async (hotelId: number, origin: string, date: string) => {
    ...
    try {
      const apiUrl = `/api/bookings/user/flight-suggests?hotelId=${hotelId}&origin=${encodeURIComponent(
        city
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        const response = await fetch(`/api/hotel/visitor/get-hotels-name?query=${searchQuery}`);
        if (!response.ok) {
          throw new Error("Failed to fetch suggestions");
        }
        const data = await response.json();

        const hotelSuggestions = data.slice(0, 5).map((hotel: { name: string; city: string; country: string }) =>
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const apiUrl = `/api/hotel/visitor/get-hotel-search?city=${encodeURIComponent(
        city
      )}&country=${encodeURIComponent(
        country
      )}&checkInDate=${encodeURIComponent(checkInDate)}&checkOutDate=${encodeURIComponent(
        checkOutDate
  ...
  React.useEffect(() => {
    if (origin?.length > 0) {
      Promise.allSettled([
        fetch(`/api/flights/cities?city=${origin}`).then((res) => res.json()),
        fetch(`/api/flights/airports?airport=${origin}`).then((res) => res.json()),
      ])
        .then((results) => {
          const cities = results[0].status === "fulfilled" ? results[0].value : [];
          const airports = results[1].status === "fulfilled" ? results[1].value : [];
          const citySuggestions = Array.isArray(cities)
            ? cities.slice(0, 5).map(({ city, country }: { city: string; country: string }) => `${city}, ${country}`)
            : [];
        ...
        .catch((err) => console.error("Error fetching origin suggestions:", err));
    } else {
  ...
  {suggestions.length > 0 && (
    <ul className="bg-white border border-gray-300 rounded shadow-md mt-2">
      ...
      {hotelInfo && (
        <div className="mt-6 border border-gray-300 rounded-lg shadow-md p-4 bg-white">
          <div className="flex items-center gap-4">
            ...
            />
            <div>
              <h3 className="text-2xl font-semibold text-indigo-600">{hotelInfo.name}</h3>
              <p className="text-gray-700">
                <span className="font-semibold">Star Rating:</span> {hotelInfo.starRating} stars
              </p>
              <p className="text-gray-700">
                <span className="font-semibold">Address:</span> {hotelInfo.address}
              </p>
            </div>
          </div>
        </div>
      ...
      {availableRooms.length > 0 && (
        <div className="mt-6">
          <h3 className="text-xl font-semibold text-gray-800 mb-4">Available Room Types</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {availableRooms.map((room, index) => (
              ...
              >
                <h4 className="text-lg font-medium text-indigo-600">{room.roomType}</h4>
                <p className="text-gray-700 mt-2">
                  <span className="font-semibold">Price:</span> ${room.pricePerNight} per night
                </p>
                <p className="text-gray-700 mt-1">
                  <span className="font-semibold">Capacity:</span> {room.roomCapacity} people
                </p>
                <p className="text-gray-700 mt-1">
...
>
  Add to Cart
   ...
   <div className="mt-6">
        <button
          onClick={() =>
            handleFlightSuggestions(hotelInfo?.id || 0, origin, checkInDate || "")
          }
            ...
            {flightSuggestions.map((flightGroup, groupIndex) => (
              <li key={groupIndex} className="border-b pb-4">
                ...
                <ul className="space-y-2">
                  {flightGroup.flights.map((flight: Flight, flightIndex:number) => (
                    <li
                      key={flightIndex}
