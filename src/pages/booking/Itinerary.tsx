...
const Itinerary: React.FC = () => {
  ...
  const [endDate, setEndDate] = React.useState<Date | null>(
    searchParams?.get("round_trip_date") ? new Date(searchParams.get("round_trip_date")!) : null
  );
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  const handlePaymentInfoSubmit = (e: React.FormEvent) => {
    ...
    // Validate passport number (alphanumeric and 6–9 characters long)
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  React.useEffect(() => {
    if (arrivalQuery?.length > 0) {
      Promise.allSettled([
        fetch(`/api/flights/cities?city=${arrivalQuery}`).then((res) => res.json()),
        fetch(`/api/flights/airports?airport=${arrivalQuery}`).then((res) => res.json()),
      ])
        .then((results) => {
        ...
        .catch((err) => console.error("Error fetching arrival suggestions:", err));
    } else {
  ...
  React.useEffect(() => {
    if (departureQuery?.length > 0) {
      Promise.allSettled([
        fetch(`/api/flights/cities?city=${departureQuery}`).then((res) => res.json()),
        fetch(`/api/flights/airports?airport=${departureQuery}`).then((res) => res.json()),
      ])
        .then((results) => {
          const cities = results[0].status === "fulfilled" ? results[0].value : [];
          const airports = results[1].status === "fulfilled" ? results[1].value : [];
          const citySuggestions = Array.isArray(cities)
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ? cities.slice(0, 5).map(({ city, country }: any) => `${city}, ${country}`)
            : [];
          const airportSuggestions = Array.isArray(airports)
        ...
        .catch((err) => console.error("Error fetching departure suggestions:", err));
    } else {
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <div className="bg-white shadow-md rounded-lg p-6 border border-gray-200">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Passport Info</h2>
            <form onSubmit={handlePaymentInfoSubmit} className="mt-4 flex flex-col gap-4">
              <div>
                <input
                  type="text"
                  id="passportNumber"
            ...
            <div className="bg-white shadow-md rounded-lg p-6 border border-gray-200">
              ...
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-gray-800">Flight Search</h2>
                <button
                  className="px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition"
                  onClick={() => setIsFlightSearchVisible(!isFlightSearchVisible)}
                >
            ...
            <div className="bg-white shadow-md rounded-lg p-6 border border-gray-200">
              {/* Collapsible Header */}
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-gray-800">Hotel Booking</h2>
                <button
                  className="px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition"
                  onClick={() => setIsHotelBookingVisible(!isHotelBookingVisible)}
                >
