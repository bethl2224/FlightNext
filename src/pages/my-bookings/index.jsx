...
const MyBookings = () => {
  ...
  const filterEmptyItineraries = () => {
    setItineraries((prevItineraries) =>
      prevItineraries.filter(
        (itinerary) =>
          itinerary.flightBooking?.flights?.length > 0 || itinerary.hotelBookings?.length > 0
      )
    );
  };
  ...
  useEffect(() => {
    const fetchItineraries = async () => {
      setLoading(true)
      try {
        const response = await fetch("/api/bookings/user/itinerary", {
          method: "GET",
          headers: {
  ...
  const cancelHotel = async (hotelId) => {
    try {
      const response = await fetch(`/api/bookings/user/hotel-cancel/${hotelId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      setItineraries((prevItineraries) =>
        prevItineraries.map((itinerary) => ({
          ...itinerary,
          hotelBookings: itinerary.hotelBookings.filter((hotel) => hotel.id !== hotelId),
        }))
      );
  ...
  const cancelAllFlights = async (itineraryId, lastName, bookingReference) => {
    try {
      const response = await fetch(
        `api/bookings/user/flight-cancel?lastName=${lastName}&bookingReference=${bookingReference}&itineraryId=${itineraryId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
